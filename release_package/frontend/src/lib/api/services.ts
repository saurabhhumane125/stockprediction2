...
export const visionApi = {
  uploadFile: async (file: File) => {
    ...
    const response = await api.post("/api/v1/vision/upload", formData, {
      headers: {
  ...
  predictVision: async (data: { filename: string; trace_id?: string }) => {
    const response = await api.post("/api/v1/vision/predict", data);
    return response.data;
  }
}
